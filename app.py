...
#app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://cho:abc123@localhost/farm'
...
def addagroproduct():
    if request.method=="POST":
        ...
        trigger = Trig(fid=current_user.id, action='added product', timestamp=str(datetime.now()))
...
def delete(rid):
    ...
    trigger = Trig(fid=post.rid, action='deleted', timestamp=str(datetime.now()))
    db.session.add(trigger)
    db.session.commit()
...
def edit(rid):
    ...
    if request.method=="POST":
        ...
        trigger = Trig(fid=post.rid, action='edited', timestamp=str(datetime.now()))
        db.session.add(trigger)
        db.session.commit()
